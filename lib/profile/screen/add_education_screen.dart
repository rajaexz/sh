...
class AddEducationScreen extends GetView<AddEducationController> {
  ...
  Widget build(BuildContext context) {
    Get.put(AddEducationController());
    return Scaffold(
      appBar: AppBar(
        title: Text("Add Education"),
      ),
      body: GetBuilder<AddEducationController>(
        init: controller,
  ...
  Widget _buildUI(){
    if(controller.isLoading){
      ...
      return AddEducationView(controller: controller,);
    }
  }
...
class AddEducationView extends StatelessWidget {
  final AddEducationController? controller;
  final _formKey = GlobalKey<FormState>();
  AddEducationView({Key? key,this.controller}) : super(key: key);
  ...
  readOnly: true,
  showTrailingIcon: false,
  hintText: "Choose Completion Date",
  label: "Completion Date",
  ...
  onTap: () async {
    DateTime? selectedDate = await showDatePicker(
      ...
      firstDate: DateTime(2000), // set a minimum year if needed
      lastDate: DateTime(2100), // set a maximum year if needed
      builder: (BuildContext context, Widget? child) {
        return Theme(
          data: Theme.of(context).copyWith(
            primaryColor: Colors.blue, // Customize the color if needed
      ...
      helpText: "Select Year", // Custom help text
      initialDatePickerMode: DatePickerMode.year, // Open the year picker first
    );

    if (selectedDate != null) {
      // Format the date to only show the year
      controller!.dateCompletionController!.text = selectedDate.year.toString();
    }
  },
  obscureText: false,
),
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          Expanded(child: GetBuilder<AddEducationController>(
            init: controller,
